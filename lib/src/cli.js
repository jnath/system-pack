...
getPackage(basePath, (err, data) => {
    ...
    systemPackager.parse((pkgName, cb) => {
        let submodulePackagePath = join(basePath, baseModulePath, pkgName);
        getPackage(submodulePackagePath, (err, pkg) => {
            ...
            console.log('mapFolder', submodulePackagePath, utils_1.mapFolder(submodulePackagePath, submodulePackagePath));
            cb(undefined, pkg);
        });
    }, (config) => {
