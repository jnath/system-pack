...
function addDeps(depName, version) {
    packageJSON.dependencies[depName] = version;
    let pkgPath = path.join(featuresPath, 'package.json');
    fs.unlinkSync(pkgPath);
    fs.writeFileSync(pkgPath, JSON.stringify(packageJSON, null, 2));
}
exports.addDeps = addDeps;
function createModule(pkg, genIndex = false, subFolder) {
    ...
    if (subFolder) {
        let mySubFolderPath = path.join(modulePath, './mysubFolder');
